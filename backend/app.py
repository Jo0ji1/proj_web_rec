...
def init_db():
    ...
    for c in ('Alimentação','Transporte','Lazer','Contas'):
        try: db.execute('INSERT INTO categories(name) VALUES(?)', (c,))
...
def get_expense(expense_id):
    ...
    row = cur.fetchone()
    return (jsonify(dict(row)),200) if row else (jsonify({'error':'Despesa não encontrada'}),404)

@app.route('/expenses', methods=['POST'])
def create_expense():
    data = request.get_json()
    valor,desc,cat,dreg = data.get('valor'), data.get('descricao'), data.get('categoria'), data.get('data_registro')
    if None in (valor,desc,cat): return jsonify({'error':'Campos faltando'}),400
    db = get_db()
    if dreg:
        cur = db.execute('INSERT INTO expenses(valor,descricao,categoria,data_registro) VALUES(?,?,?,?)', (valor,desc,cat,dreg))
    else:
        cur = db.execute('INSERT INTO expenses(valor,descricao,categoria) VALUES(?,?,?)', (valor,desc,cat))
    db.commit()
...
def update_expense(expense_id):
    ...
    db = get_db()
    if dreg:
        db.execute('UPDATE expenses SET valor=?,descricao=?,categoria=?,data_registro=? WHERE id=?', (valor,desc,cat,dreg,expense_id))
    else:
        db.execute('UPDATE expenses SET valor=?,descricao=?,categoria=? WHERE id=?', (valor,desc,cat,expense_id))
    db.commit()
